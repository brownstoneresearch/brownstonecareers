...
export function brandedEmailLayout({ title, preheader = "", content, footerNote = "", eyebrow = "Official communication" }) {
...
export function internalContactEmail({ reference, name, email, phone, inquiryType, role, subject, message } = {}) {
...
export function preScreeningResultEmail({ firstName, candidateId = "", score, grade, status = "Passed", summary = "", actionUrl, actionLabel = "Continue to the Next Stage" }) {
...
export const INVITATION_STATUS_OPTIONS = Object.freeze([
...
export const INVITATION_STAGE_OPTIONS = Object.freeze([
  {
...
export function normalizeInvitationStatus(value = "invited") {
  const key = String(value || "").trim().toLowerCase();
  return INVITATION_STATUS_MAP.has(key) ? key : "invited";
}

export function normalizeInvitationStage(value = "application_received") {
  const key = String(value || "").trim().toLowerCase();
  return INVITATION_STAGE_MAP.has(key) ? key : "application_received";
}

export function validateInvitationSelection(statusValue, stageValue) {
...
export function candidateStageInvitationEmail({
  firstName,
  role,
  candidateId,
  accessCode,
...
export function onboardingInvitationEmail({ firstName, role, candidateId, accessCode, expiresAt, portalUrl }) {
...
export function onboardingCorrectionEmail({ firstName, role, message, portalUrl }) {
  return brandedEmailLayout({
